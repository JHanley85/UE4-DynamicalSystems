...
bool FUserOnlineAccountDynSys::GetAuthAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
bool FUserOnlineAccountDynSys::GetUserAttribute(const FString& AttrName, FString& OutAttrValue) const
{
...
bool FUserOnlineAccountDynSys::SetUserAttribute(const FString& AttrName, const FString& AttrValue)
{
	const FString* FoundAttr = UserAttributes.Find(AttrName);
...
inline FString GenerateRandomUserId(int32 LocalUserNum)
{
	...
	const bool bForceUniqueId = FParse::Param( FCommandLine::Get(), TEXT( "StableDynSysID" ) );
...
bool FOnlineIdentityDynSys::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	FString ErrorStr;
	TSharedPtr<FUserOnlineAccountDynSys> UserAccountPtr;
	...
	else
	{
		...
		if (UserId == NULL)
		{
			...
			UserAccountPtr = MakeShareable(new FUserOnlineAccountDynSys(RandomUserId));
		...
		else
		{
			...
			TSharedRef<FUserOnlineAccountDynSys>* TempPtr = UserAccounts.Find(*UniqueIdStr);
			check(TempPtr);
...
bool FOnlineIdentityDynSys::Logout(int32 LocalUserNum)
{
...
bool FOnlineIdentityDynSys::AutoLogin(int32 LocalUserNum)
{
	FString LoginStr;
	FString PasswordStr;
	FString TypeStr;
...
TSharedPtr<FUserOnlineAccount> FOnlineIdentityDynSys::GetUserAccount(const FUniqueNetId& UserId) const
{
	TSharedPtr<FUserOnlineAccount> Result;

	FUniqueNetIdString StringUserId(UserId);
	const TSharedRef<FUserOnlineAccountDynSys>* FoundUserAccount = UserAccounts.Find(StringUserId);
...
TArray<TSharedPtr<FUserOnlineAccount> > FOnlineIdentityDynSys::GetAllUserAccounts() const
{
	TArray<TSharedPtr<FUserOnlineAccount> > Result;
	
	for (TMap<FUniqueNetIdString, TSharedRef<FUserOnlineAccountDynSys>>::TConstIterator It(UserAccounts); It; ++It)
	{
		Result.Add(It.Value());
	}
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityDynSys::GetUniquePlayerId(int32 LocalUserNum) const
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityDynSys::CreateUniquePlayerId(uint8* Bytes, int32 Size)
{
...
TSharedPtr<const FUniqueNetId> FOnlineIdentityDynSys::CreateUniquePlayerId(const FString& Str)
{
	return MakeShareable(new FUniqueNetIdString(Str));
}

ELoginStatus::Type FOnlineIdentityDynSys::GetLoginStatus(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
ELoginStatus::Type FOnlineIdentityDynSys::GetLoginStatus(const FUniqueNetId& UserId) const 
{
...
FString FOnlineIdentityDynSys::GetPlayerNickname(int32 LocalUserNum) const
{
	...
	return TEXT("DynSysUser");
}

FString FOnlineIdentityDynSys::GetPlayerNickname(const FUniqueNetId& UserId) const
{
	return UserId.ToString();
}

FString FOnlineIdentityDynSys::GetAuthToken(int32 LocalUserNum) const
{
	TSharedPtr<const FUniqueNetId> UserId = GetUniquePlayerId(LocalUserNum);
	if (UserId.IsValid())
	{
...
void FOnlineIdentityDynSys::RevokeAuthToken(const FUniqueNetId& UserId, const FOnRevokeAuthTokenCompleteDelegate& Delegate)
{
	UE_LOG(LogOnline, Display, TEXT("FOnlineIdentityDynSys::RevokeAuthToken not implemented"));
	TSharedRef<const FUniqueNetId> UserIdRef(UserId.AsShared());
	DynSysSubsystem->ExecuteNextTick([UserIdRef, Delegate]()
	{
		Delegate.ExecuteIfBound(*UserIdRef, FOnlineError(FString(TEXT("RevokeAuthToken not implemented"))));
	});
}

FOnlineIdentityDynSys::FOnlineIdentityDynSys(FOnlineSubsystemDynSys* InSubsystem)
	: DynSysSubsystem(InSubsystem)
{
	// autologin the 0-th player
	Login(0, FOnlineAccountCredentials(TEXT("DummyType"), TEXT("DummyUser"), TEXT("DummyId")) );
}

FOnlineIdentityDynSys::~FOnlineIdentityDynSys()
{
}

void FOnlineIdentityDynSys::GetUserPrivilege(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, const FOnGetUserPrivilegeCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(UserId, Privilege, (uint32)EPrivilegeResults::NoFailures);
}

FPlatformUserId FOnlineIdentityDynSys::GetPlatformUserIdFromUniqueNetId(const FUniqueNetId& UniqueNetId) const
{
...
FString FOnlineIdentityDynSys::GetAuthType() const
{
	return TEXT("");
}

...
#include "NboSerializerDynSys.h"



FOnlineSessionInfoDynSys::FOnlineSessionInfoDynSys() :
	HostAddr(NULL),
	SessionId(TEXT("INVALID"))
{
}

void FOnlineSessionInfoDynSys::Init(const FOnlineSubsystemDynSys& Subsystem)
{
...
/**
 *	Async task for ending a DynSys online session
 */
class FOnlineAsyncTaskDynSysEndSession : public FOnlineAsyncTaskBasic<FOnlineSubsystemDynSys>
{
private:
...
public:
	FOnlineAsyncTaskDynSysEndSession(class FOnlineSubsystemDynSys* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskDynSysEndSession()
	{
	}
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskDynSysEndSession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
/**
 *	Async task for destroying a DynSys online session
 */
class FOnlineAsyncTaskDynSysDestroySession : public FOnlineAsyncTaskBasic<FOnlineSubsystemDynSys>
{
private:
	/** Name of session ending */
	FName SessionName;
...
public:
	FOnlineAsyncTaskDynSysDestroySession(class FOnlineSubsystemDynSys* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskDynSysDestroySession()
	{
	}
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskDynSysDestroySession bWasSuccessful: %d SessionName: %s"), bWasSuccessful, *SessionName.ToString());
	}
...
bool FOnlineSessionDynSys::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		if (!Session->OwningUserId.IsValid())
		{
			...
			Session->OwningUserName = FString(TEXT("DynSysUser"));
		}
		...
		FOnlineSessionInfoDynSys* NewSessionInfo = new FOnlineSessionInfoDynSys();
		NewSessionInfo->Init(*DynSysSubsystem);
...
bool FOnlineSessionDynSys::CreateSession(const FUniqueNetId& HostingPlayerId, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	// todo: use proper	HostingPlayerId
	return CreateSession(0, SessionName, NewSessionSettings);
}

bool FOnlineSessionDynSys::NeedsToAdvertise()
{
...
bool FOnlineSessionDynSys::NeedsToAdvertise( FNamedOnlineSession& Session )
{
	// In DynSys, we have to imitate missing online service functionality, so we advertise:
...
bool FOnlineSessionDynSys::IsSessionJoinable(const FNamedOnlineSession& Session) const
{
...
uint32 FOnlineSessionDynSys::UpdateLANStatus()
{
	...
	if ( NeedsToAdvertise() )
	{
		...
		{
			FOnValidQueryPacketDelegate QueryPacketDelegate = FOnValidQueryPacketDelegate::CreateRaw(this, &FOnlineSessionDynSys::OnValidQueryPacketReceived);
...
bool FOnlineSessionDynSys::StartSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionDynSys::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
...
bool FOnlineSessionDynSys::EndSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionDynSys::DestroySession(FName SessionName, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionDynSys::IsPlayerInSession(FName SessionName, const FUniqueNetId& UniqueId)
{
	return IsPlayerInSessionImpl(this, SessionName, UniqueId);
}

bool FOnlineSessionDynSys::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionDynSys::CancelMatchmaking(int32 SearchingPlayerNum, FName SessionName)
{
...
bool FOnlineSessionDynSys::CancelMatchmaking(const FUniqueNetId& SearchingPlayerId, FName SessionName)
{
	UE_LOG(LogOnline, Warning, TEXT("CancelMatchmaking is not supported on this platform. Use CancelFindSessions."));
	TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
	return false;
...
bool FOnlineSessionDynSys::FindSessions(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionDynSys::FindSessions(const FUniqueNetId& SearchingPlayerId, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionDynSys::FindSessionById(const FUniqueNetId& SearchingUserId, const FUniqueNetId& SessionId, const FUniqueNetId& FriendId, const FOnSingleSessionResultCompleteDelegate& CompletionDelegates)
{
...
uint32 FOnlineSessionDynSys::FindLANSession()
{
	...
	FOnValidResponsePacketDelegate ResponseDelegate = FOnValidResponsePacketDelegate::CreateRaw(this, &FOnlineSessionDynSys::OnValidResponsePacketReceived);
	FOnSearchingTimeoutDelegate TimeoutDelegate = FOnSearchingTimeoutDelegate::CreateRaw(this, &FOnlineSessionDynSys::OnLANSearchTimeout);

	FNboSerializeToBufferDynSys Packet(LAN_BEACON_MAX_PACKET_SIZE);
...
bool FOnlineSessionDynSys::CancelFindSessions()
{
	uint32 Return = E_FAIL;
...
bool FOnlineSessionDynSys::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	uint32 Return = E_FAIL;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionDynSys::JoinSession(const FUniqueNetId& PlayerId, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	// Assuming player 0 should be OK here
	return JoinSession(0, SessionName, DesiredSession);
}

bool FOnlineSessionDynSys::FindFriendSession(int32 LocalUserNum, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
...
bool FOnlineSessionDynSys::FindFriendSession(const FUniqueNetId& LocalUserId, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
...
bool FOnlineSessionDynSys::FindFriendSession(const FUniqueNetId& LocalUserId, const TArray<TSharedRef<const FUniqueNetId>>& FriendList)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
	TriggerOnFindFriendSessionCompleteDelegates(0, false, EmptySearchResult);
...
bool FOnlineSessionDynSys::SendSessionInviteToFriend(int32 LocalUserNum, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	return false;
};

bool FOnlineSessionDynSys::SendSessionInviteToFriend(const FUniqueNetId& LocalUserId, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	return false;
}

bool FOnlineSessionDynSys::SendSessionInviteToFriends(int32 LocalUserNum, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	return false;
};

bool FOnlineSessionDynSys::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in DynSys subsystem
	return false;
}

uint32 FOnlineSessionDynSys::JoinLANSession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	...
	if (Session->SessionInfo.IsValid() && SearchSession != nullptr && SearchSession->SessionInfo.IsValid())
	{
		...
		const FOnlineSessionInfoDynSys* SearchSessionInfo = (const FOnlineSessionInfoDynSys*)SearchSession->SessionInfo.Get();
		FOnlineSessionInfoDynSys* SessionInfo = (FOnlineSessionInfoDynSys*)Session->SessionInfo.Get();
...
bool FOnlineSessionDynSys::PingSearchResults(const FOnlineSessionSearchResult& SearchResult)
{
	return false;
}

/** Get a resolved connection string from a session info */
static bool GetConnectStringFromSessionInfo(TSharedPtr<FOnlineSessionInfoDynSys>& SessionInfo, FString& ConnectInfo, int32 PortOverride=0)
{
	bool bSuccess = false;
...
bool FOnlineSessionDynSys::GetResolvedConnectString(FName SessionName, FString& ConnectInfo, FName PortType)
{
	bool bSuccess = false;
	// Find the session
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session != NULL)
	{
		TSharedPtr<FOnlineSessionInfoDynSys> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoDynSys>(Session->SessionInfo);
...
bool FOnlineSessionDynSys::GetResolvedConnectString(const FOnlineSessionSearchResult& SearchResult, FName PortType, FString& ConnectInfo)
{
	bool bSuccess = false;
	if (SearchResult.Session.SessionInfo.IsValid())
	{
		TSharedPtr<FOnlineSessionInfoDynSys> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoDynSys>(SearchResult.Session.SessionInfo);
...
FOnlineSessionSettings* FOnlineSessionDynSys::GetSessionSettings(FName SessionName) 
{
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
	{
...
void FOnlineSessionDynSys::RegisterLocalPlayers(FNamedOnlineSession* Session)
{
	if (!DynSysSubsystem->IsDedicated())
	{
		IOnlineVoicePtr VoiceInt = DynSysSubsystem->GetVoiceInterface();
		if (VoiceInt.IsValid())
		{
...
void FOnlineSessionDynSys::RegisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = DynSysSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
		if (!DynSysSubsystem->IsLocalPlayer(PlayerId))
		{
...
void FOnlineSessionDynSys::UnregisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = DynSysSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
...
bool FOnlineSessionDynSys::RegisterPlayer(FName SessionName, const FUniqueNetId& PlayerId, bool bWasInvited)
{
...
bool FOnlineSessionDynSys::RegisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players, bool bWasInvited)
{
	bool bSuccess = false;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
...
bool FOnlineSessionDynSys::UnregisterPlayer(FName SessionName, const FUniqueNetId& PlayerId)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	Players.Add(MakeShareable(new FUniqueNetIdString(PlayerId)));
...
bool FOnlineSessionDynSys::UnregisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players)
{
	bool bSuccess = true;
...
void FOnlineSessionDynSys::Tick(float DeltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_Session_Interface);
	TickLanTasks(DeltaTime);
}

void FOnlineSessionDynSys::TickLanTasks(float DeltaTime)
{
	LANSessionManager.Tick(DeltaTime);
}

void FOnlineSessionDynSys::AppendSessionToPacket(FNboSerializeToBufferDynSys& Packet, FOnlineSession* Session)
{
	...
	SetPortFromNetDriver(*DynSysSubsystem, Session->SessionInfo);

	// Write host info (host addr, session id, and key)
	Packet << *StaticCastSharedPtr<FOnlineSessionInfoDynSys>(Session->SessionInfo);

	// Now append per game settings
	AppendSessionSettingsToPacket(Packet, &Session->SessionSettings);
}

void FOnlineSessionDynSys::AppendSessionSettingsToPacket(FNboSerializeToBufferDynSys& Packet, FOnlineSessionSettings* SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionDynSys::OnValidQueryPacketReceived(uint8* PacketData, int32 PacketLength, uint64 ClientNonce)
{
...
void FOnlineSessionDynSys::ReadSessionFromPacket(FNboSerializeFromBufferDynSys& Packet, FOnlineSession* Session)
{
#if DEBUG_LAN_BEACON
	...
	FOnlineSessionInfoDynSys* DynSysSessionInfo = new FOnlineSessionInfoDynSys();
	DynSysSessionInfo->HostAddr = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->CreateInternetAddr();
	Packet >> *DynSysSessionInfo;
	Session->SessionInfo = MakeShareable(DynSysSessionInfo); 

	// Read any per object data using the server object
	ReadSettingsFromPacket(Packet, Session->SessionSettings);
}

void FOnlineSessionDynSys::ReadSettingsFromPacket(FNboSerializeFromBufferDynSys& Packet, FOnlineSessionSettings& SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionDynSys::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	...
	if (CurrentSessionSearch.IsValid())
	{
		...
		FNboSerializeFromBufferDynSys Packet(PacketData, PacketLength);
...
uint32 FOnlineSessionDynSys::FinalizeLANSearch()
{
	if (LANSessionManager.GetBeaconState() == ELanBeaconState::Searching)
	{
		LANSessionManager.StopLANSession();
...
void FOnlineSessionDynSys::OnLANSearchTimeout()
{
	FinalizeLANSearch();
...
int32 FOnlineSessionDynSys::GetNumSessions()
{
	FScopeLock ScopeLock(&SessionLock);
	return Sessions.Num();
}

void FOnlineSessionDynSys::DumpSessionState()
{
	FScopeLock ScopeLock(&SessionLock);
...
void FOnlineSessionDynSys::RegisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnRegisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, EOnJoinSessionCompleteResult::Success);
}

void FOnlineSessionDynSys::UnregisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnUnregisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, true);
}

void FOnlineSessionDynSys::SetPortFromNetDriver(const FOnlineSubsystemDynSys& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	auto NetDriverPort = GetPortFromNetDriver(Subsystem.GetInstanceName());
	auto SessionInfoDynSys = StaticCastSharedPtr<FOnlineSessionInfoDynSys>(SessionInfo);
	if (SessionInfoDynSys.IsValid() && SessionInfoDynSys->HostAddr.IsValid())
	{
		SessionInfoDynSys->HostAddr->SetPort(NetDriverPort);
	}
}

bool FOnlineSessionDynSys::IsHost(const FNamedOnlineSession& Session) const
{
	if (DynSysSubsystem->IsDedicated())
	{
		return true;
	...
	IOnlineIdentityPtr IdentityInt = DynSysSubsystem->GetIdentityInterface(); 

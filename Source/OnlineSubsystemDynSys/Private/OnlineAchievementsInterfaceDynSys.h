 ...
 */
class FOnlineAchievementsDynSys : public IOnlineAchievements
{
private:

	/** Structure describing a DynSys achievement */
	struct FOnlineAchievementDynSys : public FOnlineAchievement, public FOnlineAchievementDesc
	{
	};
	 ...
	 */
	struct DynSysAchievementsConfig
	{
		/** 
		 * Create a config using the default values:
		 * IniName - GEngineIni
		 */
		DynSysAchievementsConfig()
			:	IniName(GEngineIni)
			,	SectionName(TEXT("OnlineSubsystemDynSys"))
		{
		}
		...
		bool ReadAchievements(TArray<FOnlineAchievementDynSys> & OutArray)
		{
			OutArray.Empty();
			...
			for(;;++NumAchievements)
			{
				...
				FOnlineAchievementDynSys NewAch;
				NewAch.Id = Id;
				NewAch.Progress = 0.0;

				NewAch.bIsHidden = GetKey(FString::Printf(TEXT("Achievement_%d_bIsHidden"), NumAchievements)) == TEXT("true");
				// these strings could be passed to Localize() first... but is it worth it for DynSys?
	...
	TArray<FOnlineAchievementDynSys> Achievements;
...
public:
	 ...
	 */
	FOnlineAchievementsDynSys(class FOnlineSubsystemDynSys* InSubsystem);

	/**
	 * Default destructor
	 */
	virtual ~FOnlineAchievementsDynSys(){}

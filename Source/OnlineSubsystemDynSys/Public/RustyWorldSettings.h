 ...
 */
UCLASS()
class ONLINESUBSYSTEMDYNSYS_API ARustyWorldSettings : public AWorldSettings
{
	GENERATED_BODY()
...
public:
	...
	bool SerializePropValue(FBufferArchive& Ar, UProperty* Property,UObject* Obj)
	{
		...
		else if (UObjectProperty* ObjectProperty = Cast<UObjectProperty>(Property))
		{
			...
			return false;
				...
				if (StructProperty->Struct->StaticClass()==(FPoseSnapshot::StaticStruct()->GetClass()) ){
					FPoseSnapshot Pose = *StructProperty->ContainerPtrToValuePtr<FPoseSnapshot>(Obj);
	...
	bool PropChanged(int32 PropId) {
		UProperty* Property = *PropertyMap.Find(PropId);
		UObject* Obj = *PropContainerMap.Find(PropId);
		FBufferArchive bytes;
		SerializePropValue(bytes,Property,Obj);
